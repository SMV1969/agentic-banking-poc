    ...
    # route 4 : Show portfolio
        elif "portfolio" in q_lower:
        route = "customer_portfolio"
        # naive extraction: everything after "for"
        name = user_query.split("for", 1)[-1].strip()
        if not name:
            response = "Please specify a customer name, e.g., 'Show portfolio for Chiku Van'."
            with st.chat_message("assistant"):
                st.warning(response)
        else:
            details = get_customer_portfolio_structured(name)
            with st.chat_message("assistant"):
                if details["status"] == "NOT_FOUND":
                    response = f"No accounts found for customer '{name}'."
                    st.warning(response)
                else:
                    st.subheader(f"Portfolio – {details['customer_name']}")
                    st.metric("Total balance", f"{details['total_balance']:,.2f}")
                    import pandas as pd

                    df = pd.DataFrame(details["accounts"])
                    df["balance"] = df["balance"].map(lambda x: f"{x:,.2f}")
                    st.table(df.rename(columns={"account_id": "Account ID"}))
                    response = (
